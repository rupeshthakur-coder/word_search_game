...
class WordSearchGrid extends StatefulWidget {
  ...
  final List<String> gridLetters;
  final Function(List<int>) onWordSelected;
  final Set<int> correctIndices; // Add this parameter

  WordSearchGrid({
    required this.gridSize,
    required this.gridLetters,
    required this.onWordSelected,
    required this.correctIndices, // Initialize this parameter
  });
...
class _WordSearchGridState extends State<WordSearchGrid> {
  ...
  void _handleTap(int index) {
    setState(() {
      if (!widget.correctIndices.contains(index)) {
        if (selectedIndices.contains(index)) {
          selectedIndices.remove(index);
        } else {
          selectedIndices.add(index);
        ...
        itemBuilder: (context, index) {
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: isCorrect
                    ? Colors.greenAccent.withOpacity(0.7) // Mark as green if correct
                    : isSelected
                        ? Colors.blueAccent.withOpacity(0.7)
                        : Colors.white,
                border: Border.all(
